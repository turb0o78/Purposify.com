  ...
  try {
    ...
    try {
      ...
      if (!tokenResponse.ok) {
        ...
        return Response.redirect(`${APP_URL}/connections?error=${encodeURIComponent("Failed to exchange auth code: " + errorText)}`);
      }
      ...
      const { error: insertError } = await supabaseAdmin
        ...
        .insert({
          ...
          platform_user_id: userInfo.id, // Google API returns 'id' as the user identifier
      ...
      return Response.redirect(`${APP_URL}/connections?error=${encodeURIComponent(error.message || "Unknown error occurred")}`);
    }
  } catch (error) {
    console.error("Google Drive OAuth Callback Error:", error);
    return Response.redirect(`${APP_URL}/connections?error=${encodeURIComponent(error.message || "Unknown error occurred")}`);
